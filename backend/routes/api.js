  ...
  const request = https.request(options, (apiRes) => {
    ...
    apiRes.on('end', () => {
      try {
        ...
        console.log('[UptimeRobot] response stat:', parsed.stat, 'error:', parsed.error);
        if (parsed.stat !== 'ok' || !parsed.monitors?.length) {
          return res.json({ error: parsed.error || 'No monitors found', raw: parsed });
        }
        ...
        console.error('[UptimeRobot] parse error:', e.message, 'raw:', data.slice(0, 200));
        res.json({ error: 'Parse error: ' + e.message });
      }
    });
  });
  request.on('error', (e) => { console.error('[UptimeRobot] request error:', e.message); res.json({ error: e.message }); });
  request.write(body);
  request.end();
});
